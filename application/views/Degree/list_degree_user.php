    ...
    <div class="container" >
        <div class="row">
            <div class="col mt-5 mb-1">
                <h3> <i class="fa fa-university" aria-hidden="true"></i> Daftar Pendidikan</h3>
            </div>
        </div>
        <div class="row">
            <div class="col-md-4 col-sm-12">
                <div class="form-group">
                    ...
                    <select class="form-control" name="jenis" id="jenis">
                        <option value="1" selected>Pendidikan Jarak Jauh</option>
                        <option value="2">Pendidikan Profesional Universitas</option>
                    </select>
                </div>
            </div>
            <div class="col-md-4 col-sm-12">
...
$(document).ready(function(){
    ...
    function showTransaksi(){
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url(); ?>my_degree/show_data', 
            dataType : "JSON",
            data:{
            ...
            success: function(data){ 
                ...
                //     html +=  '      <div class="card-body d-flex flex-column" onclick_=goto("<?php echo base_url('exam_certification/detail/'); ?>'+item.id_customer_certification+'") >    ';
                //     html += '           <div class="row">        ';   
                ...
                $.each(data.data, function(i, item) {       
                    ...
                    html += '                       <img src="<?php echo $this->CI->config->item('bisaUrl')."degree/media/foto_universitas/";?>'+item.foto_universitas+'" alt="" style="width: 100%"> ';
                    html += '                   </div>   ';             
                    html += '                   <div class="col-sm-9 col-9 col-md-9 mt-2"> ';                  
                    html += '                        <h5 style="font-size: 1rem"> '+item.nama_degree+' </h5>  ';              
                    html += '                        <p> <i class="fa fa-university" aria-hidden="true"></i> Offered by : '+item.nama_universitas+' </p> ';      
                    html += '                    </div>    ';    
                    html += '                </div>';
                    html += '            </div>';
                    if (item.foto_kartu_ujian != null) {
                        ...
                        html += '           <a id="downloadCert" href="#" onclick=goto("<?=base_url()?>my_degree/download_kartu_ujian/'+item.foto_kartu_ujian+'") class="btn btn-primary mt-2"> ';
                        html += '           <i class="fa fa-download" aria-hidden="true"></i> Unduh Kartu ujian </a> </p></div>';
                    }
                    html += '       </div>';
                    html += '</div>';
                 });
