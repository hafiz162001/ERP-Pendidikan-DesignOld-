...
// kalo diskon pake harga basic price_discounts
if ($data['data'][0]['is_diskon'] == 1) {
  $harga = $data['data'][0]['price_discount'] + $data['data'][0]['price_bisaai'] + $data['data'][0]['price_transcripts'];
} else {
  $harga = $data['data'][0]['price'] + $data['data'][0]['price_bisaai'] + $data['data'][0]['price_transcripts'];
}
  ...
  <div class="container">
    <div class="row">
      ...
      <div class="col-lg-4 mt-2">
        ...
        <div class="card mt-3 pb-3 px-2">
          <div class="card-body">
            <h4 class="card-title poppins-600"> <i class="far fa-handshake"></i> Client </h4>
            <div class="text-center poppins-600 text-size-6"><?=$data['data'][0]['client_name'];?></div>
          </div>
        </div>
        <!-- PARTNET -->
      </div>
      ...
      <div class="col-lg-8 mt-2">
        ...
        <div class="card">
          <div class="card-body">
            <h3 class="card-title poppins-600 text-size-6"><i class="fas fa-calendar-alt"></i> Keterangan Course</h3>
            <!-- Banner -->
            <div class="text-center">
              <img id="Event" src="<?=$this->mylibrary->UrlMedia('course',$data['data'][0]['photo']);?>" style="width:50%">
            </div>
            <!-- Banner -->
            <!-- Judul -->
            <div class="row mt-4">
              <div class="col-lg-4  poppins-600 ">Nama Course</div>
              <div class="col-lg-8 text-left poppins-400 poppins-600"><?=$data['data'][0]['name'];?></div>
            </div>
            <!-- /Judul -->
            <!-- Status -->
            <div class="row mt-3">
              <div class="col-lg-4  poppins-600 ">Deskripsi</div>
              <div class="col-lg-8 text-left poppins-400"><?=$data['data'][0]['description'];?></div>
            </div>
            <!-- /Status -->
            <!-- Keterangan Sertifikat -->
            <div class="row mt-3">
              <div class="col-lg-4  poppins-600 ">Info</div>
...
$(document).ready(function(){
  ...
  $("#btnBayar").click(function(){
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url(); ?>course/bayar', 
        dataType : "JSON",
        data:{
        ...
        success: function(data){ 
            if(data.status_code == 200){
              errMsg("Course berhasil ditambahkan.");
              // setTimeout(() => {
              ...
              if (data.data.redirect_url != null) {
                ...
                errMsg("Anda sudah terdaftar dalam course ini.", "alert-danger");
