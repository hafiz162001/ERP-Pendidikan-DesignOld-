...
<div class="row mb-3">
    ...
    <div class="col-md-4">
        <select class="form-control" id="filterCourse" style="width: 100%;">
            <option value="">Semua Degree</option>
        </select>
    </div>
    <div class="col-md-4">
                ...
                <div class="row">
                    ...
                    <div class="col-md-12 col-sm-12 p-3">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="nama" placeholder="Nama Batch">
                        </div>

                        <div class="form-group">
                          <label for="">Degree</label>
                          <select class="form-control" id="degree" style="width: 100%;">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group date" id="datetimepicker1" data-target-input="nearest">
                                    <input type="text" id="tanggal_mulai_daftar" class="form-control datetimepicker-input" data-target="#datetimepicker1" placeholder="Waktu mulai pendaftaran" maxlength="10" readonly/>
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group date" id="datetimepicker2" data-target-input="nearest">
                                    <input type="text" id="tanggal_akhir_daftar" class="form-control datetimepicker-input" data-target="#datetimepicker1" placeholder="Waktu akhir pendaftaran" maxlength="10" readonly/>
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group date" id="datetimepicker3" data-target-input="nearest">
                                    <input type="text" id="waktu_ujian" class="form-control datetimepicker-input" data-target="#datetimepicker3" readonly placeholder="Waktu dimulai ujian" maxlength="16" />
                        ...
                        <div class="form-group">
                          ...
                          <select class="form-control" id="open">
                            ...
                            <option value="1">Buka Pedaftaran</option>
                          </select>
                        </div>
...
$( "#btnSave" ).click(function() {
    ...
    $.ajax({
        ...
        data:{
            jenisData: 4,
            id_dgr_degree_batch: $('#id').val(),
            id_dgr_degree: $('#degree').find(":selected").val(),
            is_open: $('#open').find(":selected").val(),
            nama_batch: $('#nama').val(),
            tanggal_mulai_daftar: $('#tanggal_mulai_daftar').val(),
            tanggal_akhir_daftar: $('#tanggal_akhir_daftar').val(),
...
$("#filterCourse").select2({
    ajax: {
        url: "<?=base_url('Dapur/master_pendidikan/')?>showdegree",
        type: "get",
        dataType: 'json',
        delay: 250,
...
$("#degree").select2({
    ajax: {
        url: "<?=base_url('Dapur/master_pendidikan/')?>showdegreeonly",
        type: "get",
        dataType: 'json',
        delay: 250,
        processResults: function (response) {
    ...
    $.ajax({
        ...
        success: function(data){ 
            $.each(data.data, function(i, item) {
                ...
                var dataSelect = $('#degree');
                $.ajax({
                    type: 'GET',
                    url: "<?=base_url('Dapur/master_pendidikan/')?>showdegreeonly?id="+item.id_dgr_degree,
                }).then(function (data) {
                ...
                $('#id').val(item.id_dgr_degree_batch);
                $('#nama').val(item.nama_batch);
                $('#tanggal_mulai_daftar').val( formatDate(item.tanggal_mulai_daftar) );
                $('#tanggal_akhir_daftar').val( formatDate(item.tanggal_akhir_daftar) );
                $('#waktu_ujian').val( formatDate(item.waktu_ujian,1) );
                $('#open').val(item.is_open);
            });
...
function formatDate(value, jenis=2){
    ...
    if (jenis == 1) {
        ...
        newDate = date.getFullYear()+'-'+String(date.getMonth()).padStart(2, "0")+'-'+String(date.getDate()).padStart(2, "0");
    }
...
function showTransaksi(page){
    $.ajax({
        ...
        data:{
            ...
            jenisData:4,
            id_degree: $('#filterCourse').find(':selected').val(),
            q:$('#search').val()
        },
        beforeSend: function(){
            $("#loading").css('display', 'flex');
        ...
        success: function(data){ 
            ...
            $.each(data.data, function(i, item) {
            html += '    <tr>';
            html += '        <td>'+item.id_dgr_degree_batch+'</td>';
            html += '       <td>'+item.nama_batch+'</td>';
            ...
            html += '         <td> ';
            if (item.is_aktif == 1) {
                html += '<a class="btn btn-sm btn-danger btnDelete" data-id="'+item.id_dgr_degree_batch+'" data-aktif="2" title="Ubah ke tidak aktif" > <i class="fa fa-trash" aria-hidden="true"></i></a>';
                html += ' <a class="btn btn-sm btn-secondary btnDelete" data-id="'+item.id_dgr_degree_batch+'" data-aktif="3" title="Ubah ke pending"> <i class="fas fa-exclamation-triangle"></i></a>';
            } else if(item.is_aktif == 2) {
                html += '<a class="btn btn-sm btn-success btnDelete" data-id="'+item.id_dgr_degree_batch+'" data-aktif="1" title="Ubah ke aktif"> <i class="fa fa-check" aria-hidden="true"></i></a>';
            } else {
                html += '<a class="btn btn-sm btn-success btnDelete" data-id="'+item.id_dgr_degree_batch+'" data-aktif="1" title="Ubah ke aktif"> <i class="fa fa-check" aria-hidden="true"></i></a>';
            } 
            html += ' <a data-id="'+item.id_dgr_degree_batch+'"  class="btn btn-sm btn-primary btnEdit" ><i class="fa fa-pencil-alt" aria-hidden="true"></i></a> </td>';
            html += '     </tr>';
            });
