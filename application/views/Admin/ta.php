...
<div class="row mb-3">
    ...
    <div class="col-md-6">
        <select class="form-control" id="final">
            <option value="1"> Hanya Yang Sudah Mengumpulkan</option>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-4 col-sm-6 mt-2" id="phone">
                        phone 
                    </div>
...
$( "#btnSave" ).click(function() {
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url('Dapur/'); ?>tugas/save_ta', 
        dataType : "JSON",
        data:{
            id_customer_course: $('#id').val(),
            score_final: $('#score_final').val(),
            [tokenName]: tokenHash
        },
        beforeSend: function(){
...
function getDataId(id){
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url('Dapur/'); ?>tugas/showdata_ta', 
        dataType : "JSON",
        data:{
            id: id
        },
        success: function(data){ 
            $.each(data.data, function(i, item) {
                ...
                $('#id').val(item.id_customer_course);
                $('#nama_course').html(item.course_name);
                $('#nama_user').html(item.customer_name);
                $('#phone').html(item.phone);
                if (item.task_final == null) {
                    $('#fileName').html('<span class="badge badge-danger">File tidak tersedia</span>');
                } else {

                    url = '<a href="<?php echo $this->CI->config->item('bisaUrl')."academy/media/task/";?>'+item.task_final+'"> '+item.task_final+' </a>';
                    $('#fileName').html(url);

                }
                
                $('#score_final').val(item.score_final);

                if (item.task_time == null) {
                    $('#start_task_time').html('-');
                } else {
                    date = new Date(item.task_time);
                ...
                if (item.task_time_submit == null) {
                    $('#submit_task_time').html('-');
                } else {
                    date = new Date(item.task_time_submit);
                    newDate = date.getFullYear()+'-'+String(date.getMonth()).padStart(2, "0")+'-'+String(date.getDate()).padStart(2, "0")+' '+String(date.getHours()).padStart(2, "0")+':'+String(date.getMinutes()).padStart(2, "0");
                    $('#submit_task_time').html(newDate);
                }
...
function showTransaksi(page){
    $.ajax({
        ...
        success: function(data){ 
            ...
            $.each(data.data, function(i, item) {
            html += '    <tr>';
            html += '        <td>'+item.id_customer_course+'</td>';
            html += '       <td>'+item.id_course+' - '+item.course_name+'</td>';
            html += '         <td>'+item.customer_name+'</td>';
            html += '         <td> '+item.score_final+'</td>';

            if (item.status == 0) {
                status = '<span class="badge badge-secondary">Belum Bayar</span>';
            } else if(item.status == 1) {
                status = '<span class="badge badge-warning">Sudah Bayar</span>';
            ...
            html += ' <a data-id="'+item.id_customer_course+'" class="btn btn-sm btn-primary btnEdit" ><i class="fa fa-eye" aria-hidden="true"></i></a> </td>';
            html += '     </tr>';
            });
