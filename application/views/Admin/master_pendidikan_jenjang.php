                ...
                <div class="row">
                    ...
                    <div class="col-md-12 col-sm-12 p-3">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="nama" placeholder="Nama Jenjang Pendidikan">
                        </div>
                         
                        <div class="form-group">
                            <label for="">Singkatan</label>
                            <input type="text" class="form-control" id="singkatan" placeholder="Singaktan" maxlength="2">
                        </div>
...
$( "#btnSave" ).click(function() {
    ...
    $.ajax({
        ...
        data:{
            jenisData: 2,
            id_dgr_jenjang_pendidikan: $('#id').val(),
            nama_jenjang_pendidikan: $('#nama').val(),
            singkatan: $('#singkatan').val(),
            foto_universitas : localImage,
            deskripsi_jenjang_pendidikan: $('#deskripsi').val(),
            [tokenName]: tokenHash
        },
        beforeSend: function(){
    ...
    $.ajax({
        ...
        success: function(data){ 
            $.each(data.data, function(i, item) {
                ...
                $('#id').val(item.id_dgr_jenjang_pendidikan);
                $('#nama').val(item.nama_jenjang_pendidikan);
                $('#singkatan').val(item.singkatan);
                $('#deskripsi').val(item.deskripsi_jenjang_pendidikan);
            });
...
function showTransaksi(page){
    $.ajax({
        ...
        success: function(data){ 
            ...
            $.each(data.data, function(i, item) {
            html += '    <tr>';
            html += '        <td>'+item.id_dgr_jenjang_pendidikan+'</td>';
            html += '       <td>'+item.nama_jenjang_pendidikan+'</td>';
            html += '       <td>'+item.singkatan+'</td>';
            ...
            html += '         <td> ';
            if (item.is_aktif == 1) {
                html += '<a class="btn btn-sm btn-danger btnDelete" data-id="'+item.id_dgr_jenjang_pendidikan+'" data-aktif="2" title="Ubah ke tidak aktif" > <i class="fa fa-trash" aria-hidden="true"></i></a>';
                html += ' <a class="btn btn-sm btn-secondary btnDelete" data-id="'+item.id_dgr_jenjang_pendidikan+'" data-aktif="3" title="Ubah ke pending"> <i class="fas fa-exclamation-triangle"></i></a>';
            } else if(item.is_aktif == 2) {
                html += '<a class="btn btn-sm btn-success btnDelete" data-id="'+item.id_dgr_jenjang_pendidikan+'" data-aktif="1" title="Ubah ke aktif"> <i class="fa fa-check" aria-hidden="true"></i></a>';
            } else {
                html += '<a class="btn btn-sm btn-success btnDelete" data-id="'+item.id_dgr_jenjang_pendidikan+'" data-aktif="1" title="Ubah ke aktif"> <i class="fa fa-check" aria-hidden="true"></i></a>';
            } 
            html += ' <a data-id="'+item.id_dgr_jenjang_pendidikan+'"  class="btn btn-sm btn-primary btnEdit" ><i class="fa fa-pencil-alt" aria-hidden="true"></i></a> </td>';
            html += '     </tr>';
            });
