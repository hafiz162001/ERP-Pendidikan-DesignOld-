                ...
                <div class="row">
                    ...
                    <div class="col-md-12 col-sm-12 p-3">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="nama" placeholder="Pertanyaan FAQ">
                        </div>

                        <div class="form-group">
                        <label for="">Jawaban</label>
                            <textarea rows="5" class="form-control" id="deskripsi" placeholder="Jawaban FAQ"></textarea>
                        </div>
...
$( "#btnSave" ).click(function() {
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url('Dapur/'); ?>cert_faq/save', 
        dataType : "JSON",
        data:{
            id_certification: $('#jenis').find(':selected').val(),
            id_certification_faq: $('#id').val(),
            pertanyaan: $('#nama').val(),
            jawaban: $('#deskripsi').val(),
            [tokenName]: tokenHash
        },
        beforeSend: function(){
...
function getDataId(id){
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url('Dapur/'); ?>cert_faq/showdata', 
        dataType : "JSON",
        data:{
            id: id,
            jenisData: 3,
        ...
        success: function(data){ 
            $.each(data.data, function(i, item) {
                ...
                $('#id').val(item.id_certification_faq);
                $('#nama').val(item.pertanyaan);
                $('#deskripsi').val(item.jawaban);
            });
            getToken();
...
function showTransaksi(page){
    $.ajax({
        ...
        success: function(data){ 
            ...
            $.each(data.data, function(i, item) {
            html += '    <tr>';
            html += '        <td>'+item.id_certification_faq+'</td>';
            ...
            html += '         <td> ';
            if (item.is_aktif == 1) {
                html += '<a class="btn btn-sm btn-danger btnDelete" data-id="'+item.id_certification_faq+'" data-aktif="2" title="Ubah ke tidak aktif" > <i class="fa fa-trash" aria-hidden="true"></i></a>';
                html += ' <a class="btn btn-sm btn-secondary btnDelete" data-id="'+item.id_certification_faq+'" data-aktif="3" title="Ubah ke pending"> <i class="fas fa-exclamation-triangle"></i></a>';
            } else if(item.is_aktif == 2) {
                html += '<a class="btn btn-sm btn-success btnDelete" data-id="'+item.id_certification_faq+'" data-aktif="1" title="Ubah ke aktif"> <i class="fa fa-check" aria-hidden="true"></i></a>';
            } else {
                html += '<a class="btn btn-sm btn-success btnDelete" data-id="'+item.id_certification_faq+'" data-aktif="1" title="Ubah ke aktif"> <i class="fa fa-check" aria-hidden="true"></i></a>';
            } 
            html += ' <a data-id="'+item.id_certification_faq+'"  class="btn btn-sm btn-primary btnEdit" ><i class="fa fa-pencil-alt" aria-hidden="true"></i></a> </td>';
            html += '     </tr>';
            });
