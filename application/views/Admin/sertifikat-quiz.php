...
<div class="row">
    ...
    <small style="color: red;">* Pilih exam dahulu untuk menambahkan data baru. </small>
</div>
<div class="row mb-3">
    <div class="col-md-4">
...
$( "#btnSave" ).click(function() {
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url('Dapur/'); ?>cert_quiz/save', 
        dataType : "JSON",
        data:{
            id_certification_exam: $('#jenis').find(':selected').val(),
            id_certification_exam_quiz: $('#id').val(),
            pertanyaan: editor.getData(),
            jawaban1: editor2.getData(),
            jawaban2: editor_salah1.getData(),
            jawaban3: editor_salah2.getData(),
            jawaban4: editor_salah3.getData(),
            jawaban5: editor_salah4.getData(),
            [tokenName]: tokenHash
        },
        beforeSend: function(){
...
$("#datatable").on("click", ".btnDelete", function(){
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url('Dapur/'); ?>cert_quiz/delete', 
        dataType : "JSON",
        data:{
            is_aktif: aktif,
...
$("#jenis").select2({
    ajax: {
        ...
        url: "<?=base_url('Dapur/cert_quiz/')?>showexam",
        type: "get",
        dataType: 'json',
        delay: 250,
...
function getDataId(id){
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url('Dapur/'); ?>cert_quiz/showdata', 
        dataType : "JSON",
        data:{
            id: id
        },
        success: function(data){ 
            $.each(data.data, function(i, item) {
                $('#id').val(item.id_certification_exam_quiz);        
                editor.setData(item.pertanyaan);
                editor2.setData(item.jawaban1);
                editor_salah1.setData(item.jawaban2);
                editor_salah2.setData(item.jawaban3);
                editor_salah3.setData(item.jawaban4);
                editor_salah4.setData(item.jawaban5);
...
function showTransaksi(page){
    $.ajax({
        ...
        success: function(data){ 
            ...
            $.each(data.data, function(i, item) {
            html += '    <tr>';
            html += '        <td>'+item.id_certification_exam_quiz+'</td>';
            html += '       <td>'+item.pertanyaan+'</td>';
            html += '         <td>A. '+item.jawaban1+' <br> B. '+item.jawaban2+' <br> C. '+item.jawaban3+' <br> D. '+item.jawaban4+' <br> E. '+item.jawaban4+'</td>';
            html += '         <td>A. '+item.jawaban1+'</td>';
            ...
            html += '         <td> ';
            if (item.is_aktif == 1) {
                html += '<a class="btn btn-sm btn-danger btnDelete" data-id="'+item.id_certification_exam_quiz+'" data-aktif="2" title="Ubah ke tidak aktif" > <i class="fa fa-trash" aria-hidden="true"></i></a>';
                html += ' <a class="btn btn-sm btn-secondary btnDelete" data-id="'+item.id_certification_exam_quiz+'" data-aktif="3" title="Ubah ke pending"> <i class="fas fa-exclamation-triangle"></i></a>';
            } else if(item.is_aktif == 2) {
                html += '<a class="btn btn-sm btn-success btnDelete" data-id="'+item.id_certification_exam_quiz+'" data-aktif="1"  title="Ubah ke aktif"> <i class="fa fa-check" aria-hidden="true"></i></a>';
            }  else {
                html += '<a class="btn btn-sm btn-success btnDelete" data-id="'+item.id_certification_exam_quiz+'" data-aktif="1"  title="Ubah ke aktif"> <i class="fa fa-check" aria-hidden="true"></i></a>';

            }
            html += ' <a data-id="'+item.id_certification_exam_quiz+'" data-name="wkwkland" class="btn btn-sm btn-primary btnEdit" ><i class="fa fa-pencil-alt" aria-hidden="true"></i></a> </td>';
            html += '     </tr>';
            });
