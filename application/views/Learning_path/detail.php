...
<?php
$kodeUnik = $kode_unik;
$harga = $data['data'][0]["price"] + $data['data'][0]["price_bisaai"];
if($data['data'][0]["is_discount"] == 1){
  $harga = $data['data'][0]["price_discount"];
} 
  ...
  <div class="container">
    <div class="row">
      ...
      <div class="col-lg-4 mt-2">
          ...
          <form action="" method="post" accept-charset="utf-8">
            ...
            <div class="card mb-5 mb-lg-0">
              <div class="card-body text-center">
                  ...
                  <div class="text-left mt-3 mb-3">
                    ...
                    <div class="card mt-3 no-bni text-left" id="bni" style="display: none;">
                      <div class="card-body">
                        ...
                        Anda akan terdaftar pada course <b class="poppins-500"><?=$data['data'][0]['nama'];?></b>, silahkan lakukan pembayaran 
                ...
                <input type="hidden" id="id_learning_path" value="<?=$data['data'][0]['id_learning_path'];?>" >
        ...
        <div class="card mt-3 pb-3 px-2">
          <div class="card-body">
            <h4 class="card-title poppins-600">  Info </h4>
            <p class="card-text">
              <i class="fa fa-university" aria-hidden="true"></i> Jumlah Course: <?=$data['data'][0]['course_count'];?> </br>
              <i class="fa fa-university" aria-hidden="true"></i> Jumlah Silabus: <?=$data['data'][0]['syllabus_count'];?> </br>
              <i class="fa fa-university" aria-hidden="true"></i> Kelas: <strong> <?php
              switch ($data['data'][0]['class']) {
                case '1':
                  $k = "Pemula";
                  break;
                case '2':
      ...
      <div class="col-lg-8 mt-2">
        ...
        <div class="card">
          <div class="card-body">
            <h3 class="card-title poppins-600 text-size-6"><i class="fas fa-calendar-alt"></i> Learning Path Detail</h3>
            <!-- Banner -->
            <div class="text-center">
              <img id="Foto" src="<?php echo $this->CI->config->item('bisaUrl')."learning_path/media/";?><?=$data['data'][0]['photo'];?>" style="width:50%">
            </div>
            <!-- Banner -->
            <!-- Judul -->
            <div class="row mt-4">
              <div class="col-lg-4  poppins-600 ">Nama</div>
              <div class="col-lg-8 text-left "><?=$data['data'][0]['name'];?></div>
            </div>
            <!-- /Judul -->
            <!-- Status -->
            <div class="row mt-3">
              <div class="col-lg-4  poppins-600 ">Detail Course</div>
              <div class="col-lg-8 text-left poppins-400">
                <ul class="list-group">
                  <?php $penomeran = 1; foreach ($data_course as $key => $v) : ?>
                  <p>
                    # <?=$penomeran?> <?=$v['course_name']?> 
                  </p>
                  <?php $penomeran++; endforeach; ?> 
                </ul>
              </div>
            </div>
            <!-- /Status -->
            ...
            <div class="row mt-4">
              <div class="col-lg-4  poppins-600 ">Deskripsi Learning Path</div>
              <div class="col-lg-8 text-left "><?=$data['data'][0]['description'];?></div>
            </div>
...
$(document).ready(function(){
  ...
  $("#btnBayar").click(function(){
    ...
    if ( $('#kupon').val() == "" && usingCoupon == 'true') {
        Msg("Masukkan kode kupon!.", "alert-danger");
        return false;
    } 
    ...
    {
        if($('#service_code').val() == ""){
          Msg("Pilih metode pembayaran dahulu.", "alert-danger");
            return false;
        } 
    }
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url(); ?>learningpath/bayar', 
        dataType : "JSON",
        data:{
            id_learning_path:$('#id_learning_path').val(),
        ...
        success: function(data){ 
            if(data.status_code == 200){
              Msg("Learning Path berhasil ditambahkan.");
              // setTimeout(() => {
              ...
              if (data.data.redirect_url != null) {
                ...
                Msg("Anda sudah terdaftar dalam Learning Path ini.", "alert-danger");
        ...
        error: function(data){
            Msg("Opps.. Error server! "+ data.responseJSON.description, "alert-danger");
            $("#btnBayar").prop( "disabled", false );
            $("#btnBayar").text("Bayar Sekarang");
