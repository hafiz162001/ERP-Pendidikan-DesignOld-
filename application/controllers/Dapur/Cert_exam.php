...
class Cert_exam extends CI_Controller {
    ...
    {
        $data['title'] = "Certificate Exam";
        $this->load->view('Admin/sertifikat-exam', $data, FALSE);
    }

    public function save()
    {
    	$data = $this->input->post();
        if ($this->input->post('id_certification_exam') == "") {
            $url = "certification/insert_certification_exam";
            $save = $this->query->post_data($url, $data);

        } else {
           
            $url = "certification/update_certification_exam";
